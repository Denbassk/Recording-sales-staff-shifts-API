    ...
    try {
        ...
        for (const [employeeId, data] of Object.entries(employeeData)) {
            ...
            if (manualAdjustmentsMap.has(employeeId)) {
                ...
                advanceAmount = manual.advance_payment || 0;
                advanceCard = Math.round(manual.advance_card || 0);
                advanceCash = Math.round(manual.advance_cash || 0);
                paymentMethod = manual.advance_payment_method || 'card';
...
app.post('/adjust-advance-manually', checkAuth, canManagePayroll, async (req, res) => {
    ...
    // Валидация сумм
    const cardAmount = Math.round(parseFloat(advance_card) || 0);
    const cashAmount = Math.round(parseFloat(advance_cash) || 0);
...
app.post('/adjust-final-payment', checkAuth, canManagePayroll, async (req, res) => {
    ...
    // Выплаты всегда целые: копейки не выдаются ни на карту, ни наличными
    const card_remainder = Math.round(Number(req.body.card_remainder) || 0);
    const cash_payout = Math.round(Number(req.body.cash_payout) || 0);
    ...
    try {
        ...
        await supabase
            ...
            .upsert({
                ...
                // PATCH_ADV_TOTAL: клиент не шлёт advanceTotal — считаем из карты и налички
                advance_payment: (corrections.advanceTotal === undefined || corrections.advanceTotal === null || corrections.advanceTotal === '')
                    ? (Math.round(Number(corrections.advanceCard) || 0) + Math.round(Number(corrections.advanceCash) || 0))
                    : Math.round(Number(corrections.advanceTotal) || 0),
                card_remainder: corrections.salaryCard || 0,
                cash_payout: corrections.salaryCash || 0,
                total_gross: _gross,
                total_deductions: _deductions,
                total_after_deductions: _gross - _deductions,
...
app.post('/fix-universal-calculations', checkAuth, canManagePayroll, async (req, res) => {
    ...
    try {
        ...
        const updateData = {
            ...
            // PATCH_ADV_TOTAL: то же самое для фиксации
            advance_payment: (calculations.advanceTotal === undefined || calculations.advanceTotal === null || calculations.advanceTotal === '')
                ? (Math.round(Number(calculations.advanceCard) || 0) + Math.round(Number(calculations.advanceCash) || 0))
                : Math.round(Number(calculations.advanceTotal) || 0),
            card_remainder: calculations.salaryCard || 0,
            cash_payout: calculations.salaryCash || 0,
            total_gross: _grossFix,
            total_deductions: _deductionsFix,
            total_after_deductions: _grossFix - _deductionsFix,
            // Флаги фиксации
            is_fixed: true,
            fixed_at: new Date().toISOString(),
            fixed_by: req.user.id,
    ...
    try {
        ...
        for (const record of backupData) {
            ...
            const dataToRestore = {
                ...
                advance_card: Math.round(parseFloat(record.advance_card) || 0),
                advance_cash: Math.round(parseFloat(record.advance_cash) || 0),
                card_remainder: Math.round(parseFloat(record.card_remainder) || 0),
                cash_payout: Math.round(parseFloat(record.cash_payout) || 0),
