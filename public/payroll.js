...
async function exportMonthlyReportToExcel() {
    ...
    for (const row of tableRows) {
        ...
        const rowData = {
            ...
            'База начислений': Math.round(basePay),
            'Ставка': Math.round(pureRate),
            'Бонус за выручку (до 16.07)': Math.round(oldRevBonus),
            'Процент с продаж': Math.round(bPercent),
            'Пакеты': Math.round(bBag),
            'Кофе': Math.round(bCoffee),
            'Кулинария': Math.round(bCulinary),
            'Бонусы всего': Math.round(extrasTotal),
            'Премирование': Math.round(manualBonus),
            'Причина премии': row.querySelector('[name="bonus_reason"]')?.value || '',
            'Всего начислено': Math.round(totalGross),
            'Депремирование': Math.round(penalty),
            'Причина депремирования': row.querySelector('[name="penalty_reason"]')?.value || '',
            'Вычет за недостачу': Math.round(shortage),
            'Детали недостачи': shortageDetails, // НОВОЕ ПОЛЕ
            'Всего вычетов': Math.round(totalDeductions),
            'К выплате после вычетов': Math.round(totalAfterDeductions),
            'Тип выплаты': paymentType,
            'Аванс (на карту)': Math.round(advanceCardAmount),
            'Аванс (наличные)': Math.round(advanceCashAmount),
            ...
            'Остаток (на карту)': Math.round(cardRemainder),
            'Зарплата (наличными)': Math.round(cashAmount),
            'ИТОГО к выплате': Math.round(cardRemainder) + Math.round(cashAmount),
    ...
    paymentSummary.push(
        ...
        { 'Показатель': 'Сумма выплат уволенным', 'Значение': Math.round(terminatedAmount) + ' грн' },
        { 'Показатель': '═══════════════════════', 'Значение': '═══════════════════════' },
        { 'Показатель': 'Аванс на карту (всего)', 'Значение': Math.round(totalAdvanceCard) + ' грн' },
        { 'Показатель': 'Аванс наличными (всего)', 'Значение': Math.round(totalAdvanceCash) + ' грн' },
        { 'Показатель': 'Остаток на карту (всего)', 'Значение': Math.round(totalCardRemainder) + ' грн' },
        { 'Показатель': 'Зарплата наличными (всего)', 'Значение': Math.round(totalCash) + ' грн' },
        { 'Показатель': '═══════════════════════', 'Значение': '═══════════════════════' },
        { 'Показатель': 'ИТОГО на карты', 'Значение': Math.round(totalCardPayments) + ' грн' },
        { 'Показатель': 'ИТОГО наличными', 'Значение': Math.round(totalAdvanceCash + totalCash) + ' грн' }
    );
...
function displayPayrollResults(calculations, summary) {
            ...
            for (const storeName of sortedStores) {
                ...
                <td style="font-size: 11px; color: var(--text-2);">${bonusDetails}</td>
...
function displayMonthlyReport(dailyData, adjustments, month, year, finalCalculations = [], cardLimits = {}) {
    ...
    dailyData.forEach(calc => {
        ...
        var _ed = employeeData[calc.employee_id];
        _ed.bPercent += Number(calc.sales_percent) || 0;
        _ed.bBag += Number(calc.bag_bonus) || 0;
        ...
        const dialogHTML = `
            ...
            ">
                ...
                <div style="margin: 20px 0;">
                    ...
                    <input type="number" id="shortageAmount" style="width: 100%; padding: 8px; border: 1px solid var(--border); border-radius: 5px;">
                    
                    <label style="display: block; margin: 10px 0 5px;">Описание (накладная, период, причина):</label>
                    <textarea id="shortageDescription" rows="3" style="width: 100%; padding: 8px; border: 1px solid var(--border); border-radius: 5px;"></textarea>
                    
                    <label style="display: block; margin: 10px 0 5px;">Вычесть из:</label>
                    <select id="shortageDeduction" style="width: 100%; padding: 8px; border: 1px solid var(--border); border-radius: 5px;">
