...
async function buildXlsxBuffer(returns) {
  const wb = new ExcelJS.Workbook();
  wb.creator = 'Shifts API';
  wb.created = new Date();
  ...
  const styleData = (sheet, startRow, numericCols = []) => {
    for (let r = startRow; r <= sheet.rowCount; r++) {
      sheet.getRow(r).eachCell((c, col) => {
        c.border = borders;
        c.alignment = numericCols.includes(col) ? center : left;
      });
    }
  };
  ...
  for (const it of allItems) {
    ...
    if (!supplierMap[supplier]) supplierMap[supplier] = { qty: 0, sum: 0, returns: new Set() };
    ...
    if (!storeMap[it.store_address]) storeMap[it.store_address] = { qty: 0, sum: 0, returns: new Set() };
    ...
    const skuKey = it.barcode + '|' + (it.product_name || '');
    if (!skuMap[skuKey]) skuMap[skuKey] = { barcode: it.barcode, name: it.product_name, qty: 0, sum: 0, returns: new Set() };
    skuMap[skuKey].qty += parseFloat(it.quantity || 0);
    skuMap[skuKey].sum += sum;
    skuMap[skuKey].returns.add(it.return_number);
  }
  ...
  s1.addRow([]);
  s1.addRow(['ТОП-10 постачальників', '']);
  s1.getRow(s1.rowCount).font = { bold: true };
  s1.addRow(['Постачальник', 'Сума ₴']);
  styleHeader(s1.getRow(s1.rowCount));
  ...
  s1.addRow([]);
  s1.addRow(['ТОП-10 магазинів', '']);
  s1.getRow(s1.rowCount).font = { bold: true };
  s1.addRow(['Магазин', 'Сума ₴']);
  styleHeader(s1.getRow(s1.rowCount));
  ...
  s2.columns = [
    ...
    { header: 'Магазин', key: 'store', width: 24 },
    { header: 'Дата', key: 'dt', width: 18 },
    { header: 'Співробітник', key: 'emp', width: 16 },
    { header: 'Позицій', key: 'cnt', width: 10 },
    { header: 'Сума ₴', key: 'sum', width: 12 },
    { header: 'Статус', key: 'st', width: 14 },
...
async function performBackup() {
  ...
  if (!folderId) throw new Error('DRIVE_BACKUP_FOLDER_ID not set');

  // Беремо всі повернення (можна обмежити періодом якщо потрібно)
  ...
  const buffer = await buildXlsxBuffer(returns || []);
  ...
  // Ротація: лишаємо 10 останніх
  const list = await drive.files.list({
    q: `'${folderId}' in parents and name contains 'Возвраты_backup_' and trashed = false`,
  ...
  return {
    ...
    keptCount: Math.min(files.length, 10)
  };
}

// Ручний запуск з адмін‑панелі
router.post('/admin/backup-returns', async (req, res) => {
  if (!req.user || !['admin', 'accountant', 'curator'].includes(req.user.role)) {
...
module.exports = { router, performBackup };
